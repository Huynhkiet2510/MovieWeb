    ...
    const fetchFavoriteList = async () => {
      try {
        ...
        const [movieRes, tvRes] = await Promise.all([
          ...
          axios.get(
            `https://api.themoviedb.org/3/account/${user.id}/favorite/tv?session_id=${session_id}`,
            { headers }
          ),
        ]);
        ...
        {favoriteList.length === 0 ? (
          ...
          <div className="grid grid-cols-6 gap-4">
            {favoriteList.map((item) => (
              <div onClick={() => navigate(`/${item.media_type}/${item.id}`)} className="bg-[#2B2C38] rounded-lg overflow-hidden hover:scale-105 transition-transform duration-200 cursor-pointer" key={item.media_type + item.id}>
                <div className="rounded-2xl">
                  ...
                  <h3 className="flex items-center truncate w-full p-2">
                    <span className="mr-2">
                      {item.media_type === "movie" ? <FaFilm className="text-yellow-300" /> : <FaTv className="text-blue-400" />}
                    </span>
